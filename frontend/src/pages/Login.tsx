...
export default function Login() {
  ...
  async function google() {
    ...
    try {
      ...
      // Success redirects to Google, so we intentionally stay busy.
    } catch (err) {
      notify(err instanceof Error ? err.message : "Google sign-in failed", "error");
      setBusy(false);
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-sm dark:bg-gray-900">
        ...
        <button
          onClick={google}
          disabled={busy}
          className="mb-4 flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
        >
